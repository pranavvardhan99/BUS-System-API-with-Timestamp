...
class TransportRegistrationRequest(BaseModel):
    ...
    transport_service_route_id: str
...
def get_transport_registration(request: TransportRegistrationRequest):
    ...
    route_id = request.transport_service_route_id
    ...
    try:
        cur.execute(
            "SELECT * FROM transport.transport_registration WHERE userid=%s AND semstr_details_semester_sub_id=%s AND transport_service_route_id=%s", (
                user_id, sem_sub_id, route_id,)
        )
    except psycopg2.Error as e:
    ...
    # If no rows were found, log error, insert into bus_qr_scan_log, and return 404 response
    if len(rows) == 0:
        try:
            # Connect to Postgres
            conn = psycopg2.connect(
            ...
            cur.execute(
                "INSERT INTO transport.bus_qr_scan_log (log_userid, semstr_details_semester_sub_id, log_timestamp) VALUES (%s, %s, CURRENT_TIMESTAMP)",
                (user_id, sem_sub_id,)
            )
            ...
            raise HTTPException(
                status_code=500,detail='You are inserting both wrong credentials again')
